			...
			if formulario.is_valid():
				...
				if date.today() <= add.fecha_devolucion:
					try: #guarda datos del bibliotecario al e
						y = Usuario.objects.get(tipo_usuario__nombre = "bibliotecario", user = request.user.id)
					...
					try:
						if request.user.is_authenticated:# and request.user_is_staff:
							add.usuario = Usuario.objects.get(user = request.user.id)
							usu = add.usuario
...
def prestar_view(request, id_prestar):  #modificado el 17 de abril Funcion prestar
	if request.user.is_authenticated and request.user.is_staff:
		...
		if usu:
			try:
				usu = Usuario.objects.get(tipo_usuario__nombre = "bibliotecario", user = request.user.id)
